...
class TestPartnerBankISR(common.SavepointCase):
    ...
    def setUp(self):
        super().setUp()
        self.abs_bank = self.env["res.bank"].create(
            {"name": "Alternative Bank Schweiz", "bic": "ABSOCH22XXX"}
        )
        self.supplier1 = self.env["res.partner"].create({"name": "Supplier ISR 1"})
        self.supplier2 = self.env["res.partner"].create({"name": "Supplier ISR 2"})
